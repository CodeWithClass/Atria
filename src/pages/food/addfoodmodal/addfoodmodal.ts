		...
		if (this.foodServ.foodNutdata) {
			try {
				...
				if (macro) {
					...
					let cals =
						4 * this.getMacroData("Protein") +
						(4 * this.getMacroData("Carbohydrate, by difference") +
							9 * this.getMacroData("Total lipid (fat)"))
					this.foodServ.foodNutdata.push({
						nutrient_id: "208",
						name: "Energy",
						derivation: "LCCS",
						group: "Proximates",
	...
	getServMeasure() {
		...
		if (this.foodServ.measures) {
			try {
				...
				if (servMesArr.label == "OZA") servMesArr.label = "oz"

				servMes =
					servMesArr.qty + " " + servMesArr.label + " (" + servMesArr.eqv + servMesArr.eunit + ")"
				return servMes
			} catch (e) {}
		}
	}
	...
	saveAndCloseModal() {
		...
		this.foodServ.foodNutdata.forEach(element => {
			...
			if (element.name == "Sodium, Na") {
				...
				if ((value > 500 && element.unit == "mg") || (value > 50 && element.unit == "g")) color = "red"
