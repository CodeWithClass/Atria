...
export class BPService {
  ...
  private withingsAuthURL: string =
    'https://account.withings.com/oauth2_user/authorize2?'
  private withingsDataUrl: string =
    'https://atria.coach/api/withings/fetchdata?'
  private withingsRefreshTokenUrl: string =
    'https://atria.coach/api/withings/refresh_token?'
  ...
  public withingsAuth() {
    ...
    const browser = this.inAppBrowser.create(
      this.withingsAuthURL + params.toString(),
      '_self'
    )
    try {
  ...
  public fetchBPdata(manual: boolean = false) {
    return new Promise((resolve, reject) => {
      ...
      params.set(
        'access_token',
        this.userStats.withingsAuth
          ? this.userStats.withingsAuth['access_token']
          : 'none'
      )
      ...
      this.http.get(url).subscribe(
        res => {
          if (res['response']['status'] == 401) return this.refreshToken()
          return resolve(res)
        },
        err => {
          console.log(err)
  ...
  public refreshToken() {
    ...
    params.set(
      'RefreshToken',
      this.userStats.withingsAuth['refresh_token'] || 'none'
    )
